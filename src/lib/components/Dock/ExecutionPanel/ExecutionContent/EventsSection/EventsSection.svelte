...
<section class="flex min-h-0 flex-1 flex-col gap-2" aria-label="Eventos">
  <header class="flex justify-between items-center">
    ...
    <span class="inline-flex items-center gap-1.5 rounded-md border border-zinc-200 px-2 py-1 text-xs font-medium text-zinc-600">
    ...
    {#if events.length}
      {#each events as event}
        <div class="border-b border-zinc-100 px-3 py-2 last:border-b-0">
          <p class="text-xs font-medium text-zinc-700">{getEventLabel(event)}</p>
