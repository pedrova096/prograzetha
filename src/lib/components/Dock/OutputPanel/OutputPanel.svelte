<script lang="ts">
  ...
  import {
    createOutputPanelData,
    getPreviousVariables,
    getTemplateLiteral,
    schema,
  } from './OutputPanel.utils';
  import {
    FormFields,
    type OutputForm,
  ...
  let { node, onSave, onClose, onDismiss }: OutputPanelProps = $props();
  ...
  const { form, errors, touched, data, setData, setFields } =
    createForm<OutputForm>({
      ...
      initialValues: createOutputPanelData(node?.data),
    });

  $effect(() => {
    setData(createOutputPanelData(node?.data));
  });
