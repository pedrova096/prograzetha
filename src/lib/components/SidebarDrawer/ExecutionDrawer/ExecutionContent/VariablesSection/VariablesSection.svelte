    ...
    {#if variables.length}
      <div class="max-h-40 overflow-y-auto">
        <table class="w-full table-fixed max-w-72 border-collapse text-left">
          <thead
            class="sticky top-0 bg-zinc-50 text-[11px] uppercase text-zinc-500"
          >
            <tr>
          ...
          <tbody class="divide-y divide-zinc-100">
            {#each variables as [name, value]}
              <tr>
                <td
                  class="truncate px-3 py-2 text-xs font-medium text-zinc-500"
                >
                  {name}
                </td>
                <td class="px-3 py-2">
                  <span
                    class="inline-flex rounded border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 text-[0.65rem] font-medium text-zinc-600"
                  >
                    {getValueTypeLabel(value)}
                  </span>
                </td>
                <td class="truncate px-3 py-2 text-xs text-zinc-900">
                  {formatValue(value)}
                </td>
              </tr>
            {/each}
